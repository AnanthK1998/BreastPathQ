...
x_train=np.zeros((2394,256,256,3))
y_train=np.zeros((2394,1))
imgdir=glob.glob("train1/*.tif")
n=0
for img in tqdm(imgdir):
    ...
    imag=cv2.imread(img)
    x_train[n]=imag
    y_train[n]=mydict[str(iname)]
    n+=1
x_val=np.zeros((185,256,256,3))
y_val=np.zeros((185,1))
imgdir=glob.glob("val/*.tif")
n=0
for img in tqdm(imgdir):
    imgname=os.path.basename(img)
    iname=os.path.splitext(imgname)[0]
